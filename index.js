...
const createValueFile = async (link, directoryPath) => {
  ...
  if (valueContent && valueContent != null && typeof valueContent != 'object') {
  ...
  let jsonObject = null;
  try {
    jsonObject = JSON.parse(valueContent); 
  } catch (error) {
    // Если строка некорректный JSON, создаем объект вручную
    jsonObject = {
      "name": valueContent 
    };
  } 
  ...
  fs.writeFileSync(valueFilePath, JSON.stringify(jsonObject), { flag: 'w' });
...
if (!fs.existsSync(path.join(dirPath, 'eval.js'))){
  const dataEvalStart = "{\n" +
  "  \"mode\": \"req/sub\",\n" +
  "  \"relationPath\": [],\n" +
  "  \"straightPath\": [],\n" +
  ...
  "//deep.select({}) \n" + 
  "//deepSub.subscribe({}) \n";
  fs.writeFileSync(path.join(dirPath, 'eval.js'), dataEvalStart);
}
